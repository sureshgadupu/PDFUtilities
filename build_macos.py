...
def build_for_architecture(arch):
    ...
    # Build command
    command = ["pyinstaller", "pdf_utility.spec", "--noconfirm", "--clean", "--log-level=INFO"]
    ...
    try:
        ...
        process = subprocess.Popen(
            command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True, bufsize=1, env=env
        )
            ...
            if os.path.exists(src_path):
                ...
                try:
                    ...
                    if os.path.exists(executable_path):
                        result = subprocess.run(["lipo", "-info", executable_path], capture_output=True, text=True, check=True)
...
def create_universal_binary():
    ...
    try:
        ...
        command = ["lipo", "-create", x86_path, arm_path, "-output", universal_path]

        print(f"Running: {' '.join(command)}")
        subprocess.run(command, check=True)

        # Verify universal binary
        result = subprocess.run(["lipo", "-info", universal_path], capture_output=True, text=True, check=True)
